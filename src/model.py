"""
Model definition for GLUE tasks
"""
...
class GLUETransformer(L.LightningModule):
    """Lightning Module for GLUE tasks with DistilBERT"""
    ...
    def on_validation_epoch_end(self):
        if self.hparams.task_name == "mnli":
            for i, output in enumerate(self.validation_step_outputs):
                ...
                preds = (
                    torch.cat([x["preds"] for x in output])
                    .detach()
                    .cpu()
                    .numpy()
                )
                labels = (
                    torch.cat([x["labels"] for x in output])
                    .detach()
                    .cpu()
                    .numpy()
                )
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.scheduler_type == "linear":
            ...
            raise ValueError(
                f"Unknown scheduler type: {self.hparams.scheduler_type}"
            )
